...
def calculate(N, way, p0, p1, p2, price, n=10000):
    ...
    for time in trange(n):
        ...
        N=min(np.sum(lj1),np.sum(lj2))
        cha=np.sum(lj1)-np.sum(lj2)
        ...
        # 成品检查
        while nums<=3:
            # print(nums) if time%1000==0 else None
            

            # 现在制作成品,针对不同的检测流程，次品率是不一样的,这里需要计算
            from Q2.cipin import cpcipin
            p_cipin=p0
            # p_cipin = (
            ...
            # 先加组装费用
            final_cost += price["cp"] * len(cp) if nums==0 else 0
            # 再加成品的检测费用(如果有)
            if way["C"]:
                final_cost += cost(
                    cp, [price["j_cp"],price["sale"]], type="cp"
                ) if nums==0 else cost(cp, [price["j_cp"],0], type="cp")
            else:
                # 如果没有检查，意味着有次品流入市场，必然支付调换费用
                diao_num = np.where(cp == 0)[0].shape[0]
                final_cost += price["diaohuan"] * diao_num
                final_cost -= price["sale"] * (np.sum(cp)) if nums==0 else 0
            ...
            if way["E"]:
                # 这时候的花费应该是零件1的检测费用加上补足零件1数量的费用
                final_cost += cost(lj1, [price["j_lj1"]])
                lj1 = np.array([1] * np.sum(lj1)) if np.sum(lj1) else np.array([])

            if way["F"]:
                # 这时候的花费应该是零件2的检测费用加上补足零件2数量的费用
                final_cost += cost(lj2, [price["j_lj2"]])
                lj2 = np.array([1] * np.sum(lj2)) if np.sum(lj2) else np.array([])
        ...
        # 输出当前的成本，观察变化
        # print(f"Iteration {time}:  final_cost: {final_cost}") if time % 1000 == 0 else None
    ...
    # 最后返回结果
    return results.mean(), results.std(),results
...
def cost(lists, price, type="lj"):
    """
    lists:待计算的序列\\
    price:费用列表,type不同，费用对应不同\\
    如果是零件，那么price=[检测费用]\\
    如果是成品，那么price=[检测费用，销售价格]\\
    type:类型，lj,cp\\
    """
    ...
    if type == "cp":
        ...
        return jiance-xiaoshou 


if __name__ == "__main__":
    # 测试代码
    N = 10000
    way={"A":False,"B":False,"C":False,"D":True,"E":False,"F":False}
    p0 = 0.1
    p1 = 0.1
    p2 = 0.1
    price = {
